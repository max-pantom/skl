...
export type ShieldAvatarPlaygroundProps = {
  /** `figma-studio` = seed `studio-max`, manual paint on, Figma 1636:464 colors. */
  preset?: "default" | "figma-studio";
  backLink?: { href: string; label: string };
  /** Extra preview: 100×100 + #1 badge like the profile header. */
  showFigmaProfileFrame?: boolean;
};
...
export function ShieldAvatarPlayground({
  preset = "default",
  backLink = { href: "/test", label: "← Studio mock (Figma 1631:506)" },
  showFigmaProfileFrame = false,
}: ShieldAvatarPlaygroundProps) {
  const isFigma = preset === "figma-studio";

  const [seed, setSeed] = useState(isFigma ? "studio-max" : "debug-seed-1");
  const [showDebug, setShowDebug] = useState(!isFigma);
  const [overrideEnabled, setOverrideEnabled] = useState(isFigma);
  ...
  const [points, setPoints] = useState(isFigma ? 6 : 5);
  const [innerRatio, setInnerRatio] = useState(isFigma ? 0.35 : 0.38);
  const [unifiedGradient, setUnifiedGradient] = useState(true);
  const [topStop0, setTopStop0] = useState("#686868");
  const [topStop1, setTopStop1] = useState(isFigma ? "#000000" : "#181818");
  const [bottomStop0, setBottomStop0] = useState("#686868");
  const [bottomStop1, setBottomStop1] = useState(isFigma ? "#000000" : "#181818");
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-8">
      <p className="text-sm text-zinc-600">
        <Link href={backLink.href} className="text-zinc-900 underline decoration-zinc-400 underline-offset-2">
          {backLink.label}
        </Link>
        {" · "}
        <Link
          href={showFigmaProfileFrame ? "/test/playground" : "/test2"}
          className="text-zinc-900 underline decoration-zinc-400 underline-offset-2"
        >
          {showFigmaProfileFrame ? "Default preset (/test/playground)" : "Figma preset (/test2)"}
        </Link>
      </p>
      ...
      <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_300px]">
        <section className="skl-surface space-y-6 p-6">
          {showFigmaProfileFrame ? (
            <div className="border-b border-zinc-200 pb-6">
              <p className="mb-3 font-mono text-[11px] uppercase tracking-[0.12em] text-zinc-500">
                Profile header frame (100×100 + rank)
              </p>
              <div className="relative mx-auto flex w-[100px] justify-center">
                ...
                >
                  <div className="-rotate-[23.45deg]">
                    <span className="whitespace-nowrap text-[12px] font-semibold leading-normal text-black opacity-20">
                      #1
                    </span>
                  </div>
                </div>
