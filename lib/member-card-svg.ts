...
/**
 * Static SVG matching {@link MemberIdCard} layout (for download / export).
 * Omits hover parallax; photo URLs are referenced as-is (may require network when viewing file).
 */
export function buildMemberIdCardSvg(props: MemberIdCardProps): string {
  const {
    ...
    role,
    avatarUrl,
    portraitBaseSize = 200,
    portraitScale = 1.75,
    portraitOffsetRight = -72,
    portraitOffsetY = 0,
    portraitOpacity = 1,
    portraitRotateDeg = 0,
    showPortrait = true,
